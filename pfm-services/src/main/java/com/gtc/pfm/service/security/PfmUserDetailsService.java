...
import com.gtc.pfm.commons.security.PfmUser;
...
public class PfmUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        try {
           ...
           logger.error("an error occured while finding user by name:"+username,e);
           throw new UsernameNotFoundException("an error occured while finding user by name:"+username,e);
        } catch (ObjectNotFoundException e) {
            logger.error("user not found by name:"+username,e);
            throw new UsernameNotFoundException("user not found by name:"+username,e);
        }
        
        Set<GrantedAuthority> authorities = initializeGarantedAuthorities(new String[]{"user","anonymous"});
        PfmUser pfmUser = new PfmUser(authorities, true, true, true, true, user);
        LoggingUtil.trace(logger, "loaded user with authoraties:"+pfmUser);
        return pfmUser;
    }
